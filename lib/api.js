...
export const API = {
  ...
  createReply(postId, content, captcha, replyToUsername) {
    return apiFetch('/posts/' + postId + '/replies', {
      method: 'POST',
      body: JSON.stringify({ content, captcha_id: captcha?.id, captcha_answer: captcha?.answer, captcha_sig: captcha?.sig, reply_to_username: replyToUsername || null }),
    });
  },
