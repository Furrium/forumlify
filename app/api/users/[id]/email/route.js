...
export async function PUT(req, { params }) {
  ...
  try {
    const u = await pool.query('SELECT password_hash FROM users WHERE id = $1', [(await params).id]);
    if (u.rows.length === 0) {
    ...
    const existing = await pool.query('SELECT id FROM users WHERE email = $1 AND id != $2', [newEmail, (await params).id]);
    if (existing.rows.length > 0) {
      return Response.json({ error: '邮箱已被占用' }, { status: 400 });
    }
    await pool.query('UPDATE users SET email = $1 WHERE id = $2', [newEmail, (await params).id]);
